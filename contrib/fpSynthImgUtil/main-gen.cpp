...
int main(int argc, char* argv[])
{
      ...
      for (unsigned i = 0; i < 3; i++)
      {
        ...
        if (i > 0)
        {
          if (c1[i] != c1[i - 1] || c2[i] != c2[i - 1])
          {
            ...
            if (!imgDiff)
            {
              std::cout << "Warning: not black and white for point " << w
                        << ", " << h << " (" << (c1[i] == c1[i - 1])
                        << (c2[i] == c2[i - 1]) << ")" << std::endl;
            }
          }
        }
      }
  ...
  if (imgVerify)
  {
    ...
    for (std::map<bool, unsigned>::iterator it = numIncorrectRes.begin();
         it != numIncorrectRes.end();
         ++it)
    {
    ...
    {
      std::cout << "Total #incorrect[undef] : " << numIncorrectUndef
                << std::endl;
    }
