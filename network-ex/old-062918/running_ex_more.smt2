...
(declare-datatype RuleMobile ((sleepm) (R1) (R2)))
(declare-datatype RuleInfra ((sleepi) (R3)))
(declare-datatype Rule ((norule) (rulem (rulemrule RuleMobile)) (rulei (ruleirule RuleInfra))))

(declare-datatype Channel ((Chnone) (Ch1) (Ch2) (Ch3)))
 
; the global policy
(declare-fun policy_mobile (Channel Int) RuleMobile)
(declare-fun policy_infra (Channel Int) RuleInfra)
...
;; node to the channel
(declare-fun node.to.channel (Int Node) Channel)
; is node assigned a channel at the given time
(define-fun is-node-assigned ((time Int) (x Node)) Bool (not (= (node.to.channel time x) Chnone)))
...
(define-fun transmits ((c Channel) (time Int) (send Node) (rcv Node)) Bool
(and
  ; send has access to the channel at time
  (= (channel_access c time) send)
  (or
  (and
    (is-mobile send)
...
(ite (is-mobile x) 
  ...
  (ite (is-sleepm (policy_mobile (node.to.channel time x) time)) 0 1)
(ite (is-infra x)
  ; sleepi costs 0, all others cost 1
  (ite (is-sleepi (policy_infra (node.to.channel time x) time)) 0 1)
; (is-base x)
  0)))
...
(and
  ...
  (= (transmit x (+ time 1)) (or (transmit x time) (exists ((c Channel) (s Node)) (and (transmit s time) (transmits c time s x)))))
  ; how energy updates
...
;; well-formedness
(assert (forall ((n Node) (time Int)) 
(=> (and (<= 0 time) (< time target_time))
    (=> (is-node-assigned time n) (= (channel_access (node.to.channel time n) time) n)))
))
(assert (forall ((c Channel) (time Int)) 
(=> (and (<= 0 time) (< time target_time) (not (= c Chnone)))
    (= (node.to.channel time (channel_access c time)) c)
)))

(declare-datatype Action ((noact) (act (act_node Node) (act_rule Rule))))
...
(define-fun policy ((c Channel) (time Int)) Action
  (ite (is-mobile (channel_access c time))
    (ite (is-sleepm (policy_mobile c time))
      noact
      (act (channel_access c time) (rulem (policy_mobile c time))))
  (ite (is-infra (channel_access c time))
    (ite (is-sleepi (policy_infra c time))
      noact
      (act (channel_access c time) (rulei (policy_infra c time))))
  noact
)))
