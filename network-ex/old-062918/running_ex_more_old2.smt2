...
; the global policy
(declare-fun policy_mobile (Int NodeMobile) RuleMobile)
(declare-fun policy_infra (Int NodeInfra) RuleInfra)
...
(and
  ...
  (and
    ...
    (or 
      ; rule R2
      (and
        (= (policy_mobile time (mnode send)) R2)
  ...
  (and
    ...
    (or 
      ; rule R3
      (and 
        (= (policy_infra time (inode send)) R3)
...
;; well-formedness
(assert (forall ((c1 Channel) (c2 Channel) (time Int))
(=> (and (<= 0 time) (< time target_time) (not (= c1 c2)))
    (not (= (channel_access c1 time) (channel_access c2 time))))))
...
(define-fun policy ((c Channel) (time Int)) Action
  (ite (is-mobile (channel_access c time))
    (ite (is-sleepm (policy_mobile time (mnode (channel_access c time))))
      noact
      (act (channel_access c time) (rulem (policy_mobile time (mnode (channel_access c time))))))
  (ite (is-infra (channel_access c time))
    (ite (is-sleepi (policy_infra time (inode (channel_access c time))))
      noact
      (act (channel_access c time) (rulei (policy_infra time (inode (channel_access c time))))))
  noact
)))
