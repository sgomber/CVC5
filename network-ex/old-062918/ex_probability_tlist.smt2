...
; connectivity
(define-fun connectivity ((send Node) (rcv Node)) Real 0.75)
...
;; list of senders
(declare-datatype TList ((tlcons (tl_send Node) (tl_time Int) (tail TList)) (tlnil)))

;; state for each Node 
; chance we have transmitted to Node at a given time
(declare-fun transmit (Node Int) TList)
; how much energy we have consumed (cumulative) by a given time
(declare-fun energy (Node Int) Real)

;; initial condition 
; no transmissions
(assert (= (transmit R 0) tlnil))
(assert (= (transmit A 0) tlnil))
(assert (= (transmit B 0) tlnil))
(assert (= (transmit C 0) tlnil))
(assert (= (transmit D 0) tlnil))
...
(and
  ...
  ;(<= (transmit x (+ time 1)) (transmit x time))
  ;(or (transmit x time) (exists ((s Node)) (and (transmit s time) (transmits time s x))))
  ; how energy updates
  (= (energy x (+ time 1)) (+ (energy x time) (energy-consumed time x)))
)
)))

;; compute chance of transmission from transmit list
(declare-fun transmit_chance (Node Int) Real)
(assert (forall ((x Node) (time Int))
  (=> (and (<= 0 time) (< time target_time))
