...
(assert (>= (transmit D target_time) 0.9))

; energy constraints
(assert (<= (+ (energy R target_time) (energy A target_time) (energy B target_time) (energy C target_time) (energy D target_time)) 8.0))
...
(declare-fun act_seq-time (Int) Int)
(declare-fun act_seq-ch (Int) Channel)

; constraints on assignment go here
(assert (forall ((time Int))
(=> (and (<= 0 time) (< time act_seq_l))
(and
  (>= (act_seq-time time) 0)
  (< (act_seq-time time) target_time)
  (< (act_seq-time time) (act_seq-time (+ time 1)))
)
)))
