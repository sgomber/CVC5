...
;(sample-type UnifReal Real 1000 (Start) 
;  ((Start Real ((/ (sample.unif 0 10000) 10000))))
;)
(declare-sort UnifReal 0)
(declare-fun sample-connectivity (Node Node Int) UnifReal)
(declare-fun sample.run (UnifReal) Real)
...
(define-fun get-act-success ((x Node) (t Int)) Bool
  ...
  (ite ((_ is act_send) x_act_at_t)
    ...
    (ite (GlobalState_prcv x (_pck x_act_at_t) t)
      ...
      (> (connectivity x y t) (sample.run (sample-connectivity x y t))) 
      false
    )
    true
  )))
