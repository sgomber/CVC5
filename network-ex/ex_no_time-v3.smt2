...
(set-option :num-samples-sat 500)
...
(declare-fun sample-connectivity (Node Node Int) UnifInt)

; connected

;(declare-fun connectivity (Node Node Int) Int)
...
(define-fun connectivity ((x Node) (y Node) (t Int)) Int
  (ite (= x R)
    (ite (= y A) 900
    (ite (= y B) 500
    (ite (= y C) 300
  ...
  (ite ((_ is act_send) x_act_at_t)
    ...
    (ite (GlobalState_prcv x (_pck x_act_at_t) t)
      ...
      (let ((y (_dst x_act_at_t)))
        (< (sample.run (sample-connectivity x y t)) (connectivity x y t))
      )
      false
    )
    true
