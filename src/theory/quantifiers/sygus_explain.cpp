...
VirtualSygusTerm* VirtualSygusTerm::addTester(TermDbSygus* tdb,
                                              TypeNode tn,
                                              Node tst)
{
  Node a;
...
void VirtualSygusTerm::clearTerm() { d_build_term = Node::null(); }
...
void VirtualSygusTerm::getSubterms(std::vector<VirtualSygusTerm*>& subterms)
{
  ...
  for (std::map<unsigned, VirtualSygusTerm>::iterator it = d_children.begin();
       it != d_children.end();
       ++it)
  {
                                     ...
                                     unsigned& sz)
{
  ...
  Trace("sygus-eq-gen-debug") << "Get explanation for " << n << " == " << vn
                              << " returned: " << std::endl;
  TypeNode tn = n.getType();
  ...
  var_count.clear();
  Trace("sygus-eq-gen-debug")
      << "Virtual sygus term built : " << vsb << std::endl;
  std::map<Node, bool> tst_gen;
  std::vector<Node> eq_gen_exp;
  for (const std::pair<Node, std::vector<VirtualSygusTerm*> >& st : subterms)
  {
    Trace("sygus-eq-gen-debug")
        << "Look at subterms for " << st.first << std::endl;
    if (st.second.size() > 1)
    {
      Node t = st.first;
      Trace("sygus-eq-gen-debug")
          << "..." << st.second.size() << " subterms for " << t << std::endl;
      ...
      for (unsigned i = 0; i < size; i++)
      {
        for (unsigned j = (i + 1); j < size; j++)
        {
          ...
          vj->setTerm(d_tdb, k);
          Trace("sygus-eq-gen-debug")
              << "  replace by fresh var " << k << std::endl;
          ...
          if (et.is_invariant(d_tdb, nt, k))
          {
            ...
            Trace("sygus-eq-gen")
                << "*** For " << n << " == " << d_tdb->sygusToBuiltin(vn) << ","
                << std::endl;
            Trace("sygus-eq-gen") << "*** Can use " << eq
                                  << " instead of explanation:" << std::endl;
