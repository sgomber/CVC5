                               ...
                               ExpMode emode)
{
      ...
      if( it!=subs.end() ){
        ...
        if( subsRep ){
          ...
          return getEntailedTerm2(
              it->second, subs, subsRep, hasSubs, exp, p, qy, emode);
        }
      }
    }
  }
  ...
  if (p)
  {
    ...
    {
      if (argst[i] != nn[i]
          || child_pfs[i]->d_id != eq::MERGED_THROUGH_REFLEXIVITY)
      {
        trivial = false;
      }
    }
...
TNode TermDb::getEntailedTerm(TNode n, eq::EqProof* p, EqualityQuery* qy)
{
  if( qy==NULL ){
    qy = d_quantEngine->getEqualityQuery();
  }
                         ...
                         bool computeFail)
{
  ...
  {
    return isEntailed2(
        n[0], subs, subsRep, hasSubs, !pol, exp, qy, emode, fails, computeFail);
  }
  ...
  else if (nk == EQUAL || nk == ITE)
  {
    ...
    for( unsigned i=0; i<2; i++ ){
      ...
      if (esuccess || (i == 1 && computeFail))
      {
        ...
        bool esuccess2 = isEntailed2(n[ch],
                                     subs,
                                     subsRep,
                                     hasSubs,
                                     reqPol,
...
bool TermDb::isEntailed(TNode n, bool pol, EqualityQuery* qy)
{
  ...
  std::vector<Node> fails;
  return isEntailed2(
      n, subs, false, false, pol, exp, qy, EXP_MODE_NONE, fails, false);
}
                        ...
                        EqualityQuery* qy)
{
  ...
  std::vector<Node> fails;
  return isEntailed2(
      n, subs, subsRep, true, pol, exp, qy, EXP_MODE_NONE, fails, false);
}
