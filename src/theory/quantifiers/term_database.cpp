...
Node TermDb::evaluateTerm2( TNode n, std::map< TNode, Node >& visited, std::vector< Node >& exp, EqualityQuery * qy, bool useEntailmentTests, bool computeExp,
                    bool reqHasTerm ) {
  ...
  //term is not known to be equal to a representative in equality engine, evaluate it
    ...
    for( unsigned i=0, nchild = n.getNumChildren(); i<nchild; i++ ){
      TNode c = evaluateTerm2( n[i], visited, tempExp, qy, useEntailmentTests, computeExp, reqHasTerm );
      if( c.isNull() ){
        ret = Node::null();
        ret_set = true;
        break;
        ...
        if( ( k==AND && c==d_false ) || ( k==OR && c==d_true ) ){
          ...
          ret = evaluateTerm2( n[ c==d_true ? 1 : 2], visited, tempExp, qy, useEntailmentTests, computeExp, reqHasTerm ); 
          ret_set = true;
          break;
        }
      }
        ...
        if( useEntailmentTests ){
          if( ret.getKind()==EQUAL || ret.getKind()==GEQ ){
            ...
            for( unsigned j=0; j<2; j++ ){
              std::pair<bool, Node> et = te->entailmentCheck(THEORY_OF_TYPE_BASED, j==0 ? ret : ret.negate() );
              if( et.first ){
      ...
      if( it!=subs.end() ){
        ...
        if( subsRep ){
          ...
          return getEntailedTerm2(
              it->second, subs, subsRep, hasSubs, exp, qy, computeExp);
        }
      }
    }
...
Node TermDb::evaluateTerm( TNode n, EqualityQuery * qy, bool useEntailmentTests,
  ...
  std::vector< Node > exp;
  return evaluateTerm2( n, visited, exp, qy, useEntailmentTests, false, reqHasTerm );
}

Node TermDb::evaluateTerm( TNode n, std::vector< Node >& exp, EqualityQuery * qy, bool useEntailmentTests,
                    bool reqHasTerm ) {
  if( qy==NULL ){
    qy = d_quantEngine->getEqualityQuery();
  }
  std::map< TNode, Node > visited;
  return evaluateTerm2( n, visited, exp, qy, useEntailmentTests, true, reqHasTerm );
}
...
TNode TermDb::getEntailedTerm( TNode n, std::map< TNode, TNode >& subs, bool subsRep, EqualityQuery * qy ) {
  ...
  std::vector<Node> exp;
  return getEntailedTerm2(n, subs, subsRep, true, exp, qy, false);
}
...
TNode TermDb::getEntailedTerm( TNode n, EqualityQuery * qy ) {
  ...
  std::map< TNode, TNode > subs;
  std::vector<Node> exp;
  return getEntailedTerm2(n, subs, false, false, exp, qy, false);
}
                         ...
                         bool computeExp)
{
  ...
  if( n.getKind()==EQUAL && !n[0].getType().isBoolean() ){
    TNode n1 =
        getEntailedTerm2(n[0], subs, subsRep, hasSubs, exp, qy, computeExp);
    if( !n1.isNull() ){
      TNode n2 =
          getEntailedTerm2(n[1], subs, subsRep, hasSubs, exp, qy, computeExp);
      if( !n2.isNull() ){
        if( n1==n2 ){
          return pol;
        }else{
    ...
    return isEntailed2(n[0], subs, subsRep, hasSubs, !pol, exp, qy, computeExp);
  ...
  //Boolean equality here
    ...
    for( unsigned i=0; i<2; i++ ){
      tempExp.clear();
      if (isEntailed2(
              n[0], subs, subsRep, hasSubs, i == 0, tempExp, qy, computeExp))
      {
        unsigned ch = ( n.getKind()==EQUAL || i==0 ) ? 1 : 2;
        bool reqPol = ( n.getKind()==ITE || i==0 ) ? pol : !pol;
        if (isEntailed2(
                n[ch], subs, subsRep, hasSubs, reqPol, tempExp, qy, computeExp))
        {
          if (computeExp)
          {
            exp.insert(exp.end(), tempExp.begin(), tempExp.end());
...
bool TermDb::isEntailed(TNode n, bool pol, EqualityQuery* qy)
{
  ...
  std::vector<Node> exp;
  std::map< TNode, TNode > subs;
  return isEntailed2(n, subs, false, false, pol, exp, qy, false);
}
                        ...
                        EqualityQuery* qy)
{
  ...
  std::map<TNode, TNode> subs;
  return isEntailed2(n, subs, false, false, pol, exp, qy, true);
}
                        ...
                        EqualityQuery* qy)
{
  ...
  return isEntailed2(n, subs, subsRep, true, pol, exp, qy, true);
}
                        ...
                        EqualityQuery* qy)
{
  ...
  std::vector<Node> exp;
  return isEntailed2(n, subs, subsRep, true, pol, exp, qy, false);
}
