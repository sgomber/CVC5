...
bool SynthEngine::checkConjecture(SynthConjecture* conj)
{
  ...
  if (!conj->needsRefinement())
  {
    ...
    Trace("cegqi-engine-debug") << "  *** Check candidate phase..." << std::endl;
    ...
    if (addedLemma)
    {
      Trace("cegqi-engine-debug") << "  ...check for counterexample." << std::endl;
      return true;
    }
    else
    {
  ...
  {
    Trace("cegqi-engine-debug") << "  *** Refine candidate phase..." << std::endl;
