    ...
    std::set<Type>& unres)
{
  ...
  // make the constant Boolean type, if applicable
  TypeNode unresCBool;
  if( sgcm!=SYGUS_GCONS_SIMPLE)
  {
    unresCBool = mkUnresolvedType("Constant_Bool", unres);
    unres_types.push_back(unresCBool);
    sdts.push_back(SygusDatatypeGenerator("Constant_Bool"));
    std::vector<TypeNode> cargsEmp;
    for( unsigned r=0; r<2; r++)
    {
      Node c = nm->mkConst(r==0);
      std::stringstream ss;
      ss << c;
      sdts.back().d_sdt.addConstructor(c,ss.str(),cargsEmp);
    }
    sdts.back().d_sdt.initializeDatatype(btype, bvl, true, true);
  }
  ...
  for (unsigned i = 0, size = sygus_vars.size(); i < size; ++i)
  {
    if( sygus_vars[i].getType().isBoolean() ){
      bvars.push_back(sygus_vars[i]);
    }
  }
  Trace("sygus-grammar-def") << "We have " << bvars.size() << " Boolean variables" << std::endl;
  if (sgcm!=SYGUS_GCONS_SIMPLE && bvars.size()>1)
  {
    // Given Boolean variables A1, ..., An, we add a single constructor
    // corresponding to a conjunction of (negations) of these variables. This
    // is encoded as the following:
    // ((B^+_1 ^ ~B^-_1) => A1 ^ (~B^+_1 ^ B^-_1) => ~A1 ^
    //  ...
    //  (B^+_n ^ ~B^-_n) => An ^ (~B^+_n ^ B^-_n) => ~An)
    // where B^+_1, B^-_1, ..., B^+_n, B^-_n are generated by the constant
    // Boolean grammar.
    std::vector< Node > lchildren;
    std::vector< Node > lvars;
    std::vector< TypeNode > cargsVC;
    for (unsigned i = 0, size = bvars.size(); i < size; ++i)
    {
      Node vpos = nm->mkBoundVar(btype);
      lvars.push_back(vpos);
      cargsVC.push_back(unresCBool);
      Node vneg = nm->mkBoundVar(btype);
      lvars.push_back(vneg);
      cargsVC.push_back(unresCBool);
      Node posConj = nm->mkNode(OR,vpos.negate(),vneg,bvars[i]);
      lchildren.push_back(posConj);
      Node negConj = nm->mkNode(OR,vpos,vneg.negate(),bvars[i].negate());
      lchildren.push_back(negConj);
    }
    Node lbvl = nm->mkNode(BOUND_VAR_LIST,lvars);
    Node body = nm->mkNode(AND,lchildren);
    Node op = nm->mkNode(LAMBDA,lbvl,body);
    std::shared_ptr<SygusPrintCallback> spc;
    std::vector<Expr> lambdaVarsExpr;
    for (unsigned i = 0, nvars = lvars.size(); i < nvars; i++)
    {
      lambdaVarsExpr.push_back(lvars[i].toExpr());
    }
    spc = std::make_shared<printer::SygusExprPrintCallback>(
        body.toExpr(), lambdaVarsExpr);
    Trace("sygus-grammar-def") << "Made variable conjunction operator " << op << std::endl;
    sdtBool.addConstructor(op, "Variable_Conjunction", cargsVC, spc);
  }
  else
  {
    for (unsigned i = 0, size = bvars.size(); i < size; ++i)
    {
      std::stringstream ss;
      ss << bvars[i];
      Trace("sygus-grammar-def") << "...add variable " << ss.str() << std::endl;
      std::vector<TypeNode> cargsEmpty;
      // make boolean variables weight as non-nullary constructors
      sdtBool.addConstructor(bvars[i], ss.str(), cargsEmpty, nullptr, 1);
    }
