...
class SynthConjecture
{
 ...
 private:
   ...
   * datatypes solver, hence this method is a no-op for cases where solutions
   * are being generated via active enumerators.
   *
   * If sendLemma is false, we return the lemma that forces the current solution
   * to be blocked. Similar as above, this may be null if the current solution
   * was generated by an active enumerator.
   */
  Node excludeCurrentSolution(const std::vector<Node>& enums,
                              const std::vector<Node>& values,
                              bool sendLemma = true);
  /**
