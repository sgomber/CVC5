...
void InferenceManager::sendDtConflict(const std::vector<Node>& conf, InferenceId id)
{
  ...
  // this ensures proper explanations to avoid CNF issues, although fails 6 regressions when dt has CEE, also wrong "sat" answers on Facebook v1
  // also leads to proof checking failures
