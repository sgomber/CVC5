...
AnalyzeModel::AnalyzeModel(Valuation val, RelevanceManager* rm, TheoryModel* tm)
    : d_val(val), d_rm(rm), d_model(tm)
{
  Assert(d_rm != nullptr);
}
...
void AnalyzeModel::analyzeModelFailure()
{
  ...
  if (!rsuccess)
  {
    Trace("analyze-model") << "AnalyzeModel::analyzeModelFailure: failed to "
  ...
  for (TNode l : lits)
  {
    ...
    if (d_val.getDecisionLevel(l) == 0 && d_val.getIntroLevel(l) == 0)
    {
      ...
      Trace("analyze-model-debug") << "(entailed-debug"
                                   << " " << l << ")" << std::endl;
  ...
  for (size_t i = 0, nlits = lvec.size(); i < nlits; i++)
  {
    ...
    Trace("analyze-model") << " "
                           << printPolarity(expectedVals[i],
                                            expectedValKnown[i])
                           << ids[i];
    ssd << " " << printPolarity(expectedVals[i], expectedValKnown[i])
        << lvec[i];
  }
