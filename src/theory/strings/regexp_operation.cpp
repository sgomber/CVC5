...
RegExpOpr::IntersectFrame::IntersectFrame(RegExpOpr& reo,
                                          Node r1,
                                          Node r2,
                                          unsigned cnt)
...
bool RegExpOpr::IntersectFrame::processNext(
    RegExpOpr& reo,
    ...
    std::vector<IntersectFrame>& toProcess)
{
  ...
  rt = nm->mkNode(
      REGEXP_CONCAT, nm->mkNode(STRING_TO_REGEXP, nm->mkConst(c)), rt);
