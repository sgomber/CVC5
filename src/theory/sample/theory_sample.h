 ...
 * below.
 * 
 * This theory has a notion of "sample type". All other types we call
 * ordindary types. We identify sample types with a context-free grammar,
 * which will determine the semantics of the type.
 * 
 * A "base sample term" is a term of the form (f c1 ... cn) whose return type is
 * a sample type, and c1...cn are constant terms whose type is ordinary.
 *
 * Given an interpretation M for all functions whose return type is not a sample
 * type, we say a "sample run" (for M) is an interpretation where all
 * base sample terms t are interpreted as r^M, where r is a random term
 * generated by the context free grammar associated with its sample type.
 * 
 * For example:
 *   SampleInt -> 0 | 1 | x | f( SampleInt )
 * where x : Int and f : Int -> Int, s : SampleInt and interpretation M where 
 * x^M = 7 and f^M = lambda y. 10, in a sample run of M, the value of s^M
 * is determined by first generating a random term in the above grammar, 
 * say f( x ). Then s^M = f( x )^M = 10.
 * 
 * The satisfiability check that this theory can answer is parameterized by
 * two values:
 * (1) #runs, the total number of sample runs (--num-samples),
 * (2) #sat-runs, the total number of runs that we require the input formula to
 * be satisfied in (--num-samples-sat).
 * A formula F is satisfied by an interpretation M iff F evaluates to true
 * in at least #sat-runs of #runs sample runs. Notice that this semantics is
 * non-deterministic: two checks may produce "sat" and "unsat" answers. 
 * 
