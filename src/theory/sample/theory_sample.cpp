...
void TheorySample::registerSampleTerm(Node n,
                                      std::vector<Node>& freeTerms,
                                      std::vector<Node>& sampleTerms)
{
  // ensure that d_hasSample and d_isSample have been computed for all
  // subterms
...
void TheorySample::checkLastCall()
{
  ...
  Trace("sample-conflict") << "[[make conflict]]" << std::endl;
  // also must explain model value equalities
  for (const Node& ca : d_conflict)
  {
    cvec.push_back(ca.negate());
    Trace("sample-conflict") << "  " << ca << std::endl;
    AssertInfo& cai = d_ainfo[ca];
    // it depends on the free terms of the literal
    ...
    // it also depends on the non-constant terms that were generated by
    // the base sample terms of this literal
    for (const Node& st : cai.d_sample_terms)
    {
      Node bst = d_bmv[st];
      Trace("sample-conflict-debug")
          << "    - check base sample term " << bst << "..." << std::endl;
      std::vector<Node>& bstNonConstTerms = d_bst_to_nconst_terms[bst];
      for (const Node& bstt : bstNonConstTerms)
      {
        Trace("sample-conflict") << "    sampled term : " << bstt
                                 << " from base term " << bst << std::endl;
        std::vector<Node> cvecc;
...
Node TheorySample::explainModelValue(Node n,
                                     std::vector<Node>& exp,
                                     unsigned ind)
{
  if (n.isConst())
  ...
  if (d_masterEe->hasTerm(mn))
  {
    ...
    if (d_masterEe->hasTerm(n))
    {
      ...
      indent("sample-conflict-debug", ind);
      Trace("sample-conflict-debug")
          << "-- trivial, ee has both terms." << std::endl;
      return mn;
    }
    if (n.getKind() == APPLY_UF)
    {
      ...
      if (success && childChanged)
      {
        ...
        if (!n2.isNull())
        {
          ...
          indent("sample-conflict-debug", ind);
          Trace("sample-conflict-debug")
              << "-- explained children and explanation via " << n2 << "."
              << std::endl;
          return n2;
        }
      }
  ...
  if (n.getType().isInterpretedFinite())
  {
    indent("sample-conflict-debug", ind);
    Trace("sample-conflict-debug")
        << "-- finite, allow value explanation." << std::endl;
    return mn;
  }
  indent("sample-conflict-debug", ind);
    ...
    for (unsigned k = 0, size = d_asserts.size(); k < size; k++)
    {
      ...
      for (unsigned j = 0, nvars = bt_vars.size(); j < nvars; j++)
      {
        ...
        if (itbvi == sbtvi.end())
        {
          ...
          // if this is the first occurrence of sub
          if (d_bst_term_to_samples[var][sub].size() == 1)
          {
            if (!sub.isConst())
            {
      ...
      Trace("sample-check-debug2") << "Evaluate " << ba << "{" << bt_vars
                                   << " -> " << bt_subs << "}" << std::endl;
      ...
      {
        Trace("sample-check-debug2")
            << "...requires substitution + rewriting" << std::endl;
...
Node TheorySample::getBaseModelValue(Node n)
{
  ...
  do
  {
    ...
    else if (it->second.isNull())
    {
      ...
      {
        if (std::find(
                d_base_sample_terms.begin(), d_base_sample_terms.end(), ret)
            == d_base_sample_terms.end())
        {
